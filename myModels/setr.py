...
class ViT(nn.Module):
    ...
    def forward(self, img):
        # print(f'VIT 1: {img.shape}')
        x = self.to_patch_embedding(img)
        # print(f'VIT 2: {x.shape}')
        b, n, _ = x.shape
        cls_tokens = repeat(self.cls_token, '1 1 d -> b 1 d', b=b)
        x = torch.cat((cls_tokens, x), dim=1)
        # print(f'VIT 3: {x.shape}')
        x += self.pos_embedding[:, :(n + 1)]
        # print(f'VIT 4: {x.shape}')
        x = self.dropout(x)
        # print(f'VIT 5: {x.shape}')
        ...
        for index, transformer_out in enumerate(out):
            ...
            # print(f'VIT 6 {index}: {out[index].shape}')
...
class SETR(nn.Module):
    def __init__(self, num_classes=1, image_size=256, patch_size=256//16, dim=1024, depth=24, heads=16, mlp_dim=2048, channels=3, dim_head=64,
    ...
    def forward(self, x):
        ...
        for index, indices in enumerate(self.out_indices):
            ...
            # print(f'2 index {index}: {VIT_OUT[index].shape}')
        return out
